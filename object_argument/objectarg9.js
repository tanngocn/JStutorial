...
var getPlayerItems = function (player) {
  return player.name  + player.items;
};
...
var getPlayerInfo = function (player, character) {  
  ...
  var item1=items.substr(0,items.indexOf(","));
     item2=items.substr(items.indexOf(",")+2,player.items.length);
  ...
  info += spacer.newLine() + spacer.wrap(item1, longest, character); 
  info += spacer.newLine() + spacer.wrap(item2, longest, character);
